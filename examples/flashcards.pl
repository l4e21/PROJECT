...
%@ Correct to: "flashcards_example:flashcard_factory_init(FactoryID,FactorySlots)"? yes
%@ Correct to: "flashcards_example:make_card(FactoryID,CardID,CardSlots)"? yes
%@ FactoryID = obj_1,
%@ FactorySlots = [make_card(_, _A, _B)-call_slot(make_obj(root, [(card_name(_, _B), true)], _A))],
%@ CardID = obj_2,
%@ CardSlots = [card_name(_, ren2)-true].

% ?- slot(obj_3, Head, Body).
